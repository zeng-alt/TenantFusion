<template>
  <CommonPage back>
    <NSpin :show="loading">
      ...
      <NEmpty v-else-if="!loading" class="py-40" description="配置表单不存在或已被删除" />
    </NSpin>
    ...
    <FormPreviewRenderer
      ...
      :label-placement="previewSettings.labelPlacement"
      :label-width="previewSettings.labelWidth"
      :label-align="previewSettings.labelAlign"
      :size="previewSettings.formSize"
    />
  </CommonPage>
</template>
...
import api from './api'
import FormPreviewRenderer from './components/renderer/FormPreviewRenderer.vue'

defineOptions({ name: 'FormConfigDetail' })
...
const previewSettings = ref({ labelPlacement: 'left', labelWidth: 90, labelAlign: 'right', formSize: 'medium' })
...
async function handlePreview(row) {
  try {
    ...
    previewTitle.value = `${template.value?.name || '配置表单'} - v${row.version}`
...
function openDesigner(row) {
  const { href } = router.resolve({
    ...
    query: {
      id: row.versionId,
      formConfigId: template.value.formConfigId,
